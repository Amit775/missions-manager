...
export interface IReader<T, ID = ObjectId> {
  ...
  findMany$(filter?: Filter<T>, options?: FindOptions): Observable<T[]>;
}

export interface IWriter<T, ID = ObjectId> {
  createOne$(item: T, options?: BulkWriteOptions): Observable<T>;
  createMany$(items: T[], options?: BulkWriteOptions): Observable<T[]>;
  updateOne$(
    _id: ID,
  ...
  deleteMany$(filter: Filter<T>, options?: DeleteOptions): Observable<boolean>;
}
...
export class BaseRepository<T, ID = ObjectId> implements IReader<T, ID>, IWriter<T, ID> {
  ...
  createMany$(items: T[], options?: BulkWriteOptions): Observable<T[]> {
    return from(
      this.collection.insertMany(items as OptionalId<T>[], options),
    ).pipe(
      map((result) => [
